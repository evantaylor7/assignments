...
function numThenChar(arr) {
	...
	const sortedLets = lets.sort()
	let sortedFull = sortedNums.concat(sortedLets)
	for(let i = 0; i < arrLengths.length; i++){
		finalArrs.push(sortedFull.slice(0, arrLengths[i]))
		sortedFull = sortedFull.slice(arrLengths[i])
	}
	return finalArrs
}
